...
export default function AtelierNarrativo() {
  ...
  const [universes, setUniverses] = useLocalStorage("universes", seedUniverses);
  const [sagas, setSagas] = useLocalStorage("sagas", seedSagas);
  const [books, setBooks] = useLocalStorage("books", seedBooks);
  const [bookActs, setBookActs] = useLocalStorage("bookActs", seedBookActs);
  const [chapters, setChapters] = useLocalStorage("chapters", seedChapters);
  const [characters, setCharacters] = useLocalStorage("characters", seedCharacters);
  const [universeEntries, setUniverseEntries] = useLocalStorage("universeEntries", seedUniverseEntries);
  const [events, setEvents] = useLocalStorage("events", seedEvents);
  const [appearances, setAppearances] = useLocalStorage("appearances", seedAppearances);
  const [storyEvents, setStoryEvents] = useLocalStorage("storyEvents", seedStoryEvents);
  const [locations, setLocations] = useLocalStorage("locations", seedLocations);
  const [eraConfig, setEraConfig] = useLocalStorage("eraConfig", seedEraConfig);
  const [borders, setBorders] = useLocalStorage("borders", seedBorders);
  const [loreEntries, setLoreEntries] = useLocalStorage("loreEntries", seedLore);
  const [bestiary, setBestiary] = useLocalStorage("bestiary", seedBestiary);
  const [corkNotes, setCorkNotes] = useLocalStorage("corkNotes", seedCork);
  const [ideas, setIdeas] = useLocalStorage("ideas", seedIdeas);
  const [surveys, setSurveys] = useLocalStorage("surveys", seedSurveys);
  const [relPositions, setRelPositions] = useLocalStorage("relPositions", {}); // sagaId -> {charId:{x,y}}
  const [shelfLayout, setShelfLayout] = useLocalStorage("shelfLayout", {}); // bookId -> {shelf,x,y}
  const [shelfDecor, setShelfDecor] = useLocalStorage("shelfDecor", []); // {id,kind,x,y}
  const [lastEditedBookId, setLastEditedBookId] = useLocalStorage("lastEditedBookId", null);
